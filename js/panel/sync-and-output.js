    ...
    function nextSeq() {
      // Séquence horodatée-monotone. Un simple compteur repart à 0 à chaque chargement du
      // panneau (Ctrl+R, réouverture) ; les affichages déjà ouverts gardent, eux, un lastSeq
      // élevé et rejettent alors TOUTE nouvelle mise à jour comme périmée (acceptSeq :
      // seq <= lastSeq) — d'où « la connexion se coupe » tant qu'on ne recharge pas l'écran.
      // En plancherant sur Date.now(), la séquence est toujours croissante d'un chargement à
      // l'autre, donc les écrans acceptent la reprise sans rechargement manuel.
      messageSeq = Math.max(messageSeq + 1, Date.now());
      return messageSeq;
    }
    ...
    // Coalesce replays that land within a short window into ONE broadcast: several
    // displays announcing at once (each HELLO triggers a replay) would otherwise emit one
    // SYNC_STATE per HELLO. The 100 ms window is far below the deliberate 0/350/1200 ms
    // relay-open cascade, so those stay distinct. A single broadcast reaches every display.
    let replayDebounceTimer = null;
    let replayPendingReason = '';
    function replayDisplaySyncState(reason = 'display-sync') {
      if (!stateReady) {
        pendingHello = true;
        return;
      }
      replayPendingReason = reason;
      if (replayDebounceTimer) return;
      replayDebounceTimer = setTimeout(() => {
        replayDebounceTimer = null;
        updateAiRelayDebugState({ action: `display state replayed (${replayPendingReason})` });
        sendSyncState();
      }, 100);
    }
    ...
    function sendRelayStatePush({ includeSongs = false, includeBibles = false, bumpUpdatedAt = true } = {}) {
      ...
      if (includeBibles) {
        // Full records from IndexedDB, NEVER from the in-memory `bibles` — under lazy
        // loading most versions are null sentinels there, and the receiver CLEARS its
        // bible store before writing what we send, so pushing sentinels would erase its
        // whole library. IndexedDB always holds the complete parsed verses.
        dbGetAll(STORE_BIBLES).then(records => {
          payload.bibleRecords = Array.isArray(records) ? records : [];
          relaySend(payload);
        }).catch(() => { relaySend(payload); });
        return true;
      }
      relaySend(payload);
      return true;
    ...
    function captureLiveRenderUiSnapshot() {
      ...
      return {
        ...
        letterSpacing: pickNumber(liveProjection.letterSpacing, 0),
    ...
    // hex + 0..1 alpha -> rgba(). Kept simple; falls back to the raw value if unexpected.
    function bspHexToRgba(hex, alpha) {
      const m = /^#?([0-9a-f]{6})$/i.exec(String(hex || '').trim());
      if (!m) return hex || 'rgba(0,0,0,0.55)';
      const n = parseInt(m[1], 16);
      const a = Math.max(0, Math.min(1, Number.isFinite(alpha) ? alpha : 1));
      return `rgba(${(n >> 16) & 255},${(n >> 8) & 255},${n & 255},${a})`;
    }

    // Progressive contour = a soft text outline made of layered text-shadows in a ring around
    // the glyphs. The blur (≈ size·0.55) is what makes it "progressive" — the edge fades out
    // rather than being a hard stroke. Returns the shadow layer strings (no `text-shadow:`),
    // so it can be merged with the drop shadow into one property.
    function buildContourLayers(color, size) {
      const r = Math.max(0, Math.min(20, Number(size) || 0));
      if (r <= 0 || !color) return [];
      const blur = Math.max(0.5, r * 0.55);
      const N = 12;
      const layers = [];
      for (let i = 0; i < N; i++) {
        const a = (2 * Math.PI * i) / N;
        const dx = +(r * Math.cos(a)).toFixed(2);
        const dy = +(r * Math.sin(a)).toFixed(2);
        layers.push(`${dx}px ${dy}px ${blur}px ${color}`);
      }
      return layers;
    }

    function buildContentShadowStyle(enabled, isBible) {
      const layers = [];
      if (enabled) {
        const color = isBible ? verseShadowColor : songShadowColor;
        const blur = Math.max(0, Math.min(80, Number(isBible ? verseShadowBlur : songShadowBlur) || 0));
        const x = Math.max(-40, Math.min(40, Number(isBible ? verseShadowOffsetX : songShadowOffsetX) || 0));
        const y = Math.max(-40, Math.min(40, Number(isBible ? verseShadowOffsetY : songShadowOffsetY) || 0));
        const rawOpacity = isBible ? verseShadowOpacity : songShadowOpacity;
        const opacity = Math.max(0, Math.min(1, (rawOpacity == null || isNaN(Number(rawOpacity))) ? 0.85 : Number(rawOpacity)));
        layers.push(`${x}px ${y}px ${blur}px ${bspHexToRgba(color, opacity)}`);
      }
      const contourOn = isBible ? verseContourEnabled : songContourEnabled;
      if (contourOn) {
        // Contour layers come AFTER the drop shadow so the outline paints on top of the halo.
        buildContourLayers(isBible ? verseContourColor : songContourColor,
                           isBible ? verseContourSize : songContourSize).forEach(l => layers.push(l));
      }
      let style = layers.length ? `text-shadow:${layers.join(',')};` : 'text-shadow:none;';
      // Simple stroke = a crisp outline via -webkit-text-stroke, painted BEHIND the fill
      // (paint-order:stroke fill) so thin glyphs are not eaten. Separate property, so it
      // stacks with the shadow/contour above.
      const strokeOn = isBible ? verseStrokeEnabled : songStrokeEnabled;
      if (strokeOn) {
        const sw = Math.max(0, Math.min(20, Number(isBible ? verseStrokeSize : songStrokeSize) || 0));
        const sc = (isBible ? verseStrokeColor : songStrokeColor) || '#000000FF';
        if (sw > 0) style += `-webkit-text-stroke:${sw}px ${sc};paint-order:stroke fill;`;
      }
      return style;
    }

    // Builds the payload for the CURRENT design state and returns it (null when there
    // is nothing to show). Content selection — which item, which page — happens here,
    // so calling it repeatedly under different designs yields the same verse rendered
    // differently, which is what the per-output design map needs.
    // Callers should use pushLiveUpdate(); this one only builds.
    function buildLivePayload() {
      if (!isLive || !livePointer) return null;
      ...
      if (liveItem._metaKind === 'section') return null; // séparateur : jamais projeté
      ...
      const p = pages[liveLineCursor]; if (!p) return null;
      // SD (side-screen) renders with the full-screen layout, just confined to one half by
      // the display. So every FS/LT branch below must treat SD like 'full'; only the
      // payload's `mode` field carries the real sd-left/sd-right value (+ sdWidth) so the
      // display knows to confine it.
      const isSdRatio = (effectiveLiveRatio === 'sd-left' || effectiveLiveRatio === 'sd-right');
      const mode = isSdRatio ? 'full' : effectiveLiveRatio;
      ...
      // Content shadow, chosen by kind: Bible verses use verse*, songs use song*.
      const shadowIsBible = livePointer.kind === 'bible';
      const shadowOn = shadowIsBible ? verseShadowEnabled : songShadowEnabled;
      const verseShadowStyle = buildContentShadowStyle(shadowOn, shadowIsBible);
      ...
      const payload = {
        text: outHtml,
        mode: effectiveLiveRatio,
        sdWidth: Math.max(15, Math.min(90, Number(sdWidth) || 50)),
        sdMargin: Math.max(0, Math.min(40, Number(sdMargin) || 0)),
        sdMarginTop: Math.max(0, Math.min(40, Number(sdMarginTop) || 0)),
        sdMarginRight: Math.max(0, Math.min(40, Number(sdMarginRight != null ? sdMarginRight : sdMargin) || 0)),
        sdMarginBottom: Math.max(0, Math.min(40, Number(sdMarginBottom) || 0)),
        sdMarginLeft: Math.max(0, Math.min(40, Number(sdMarginLeft != null ? sdMarginLeft : sdMargin) || 0)),
        sdSlant: Math.max(-40, Math.min(40, Number(sdSlant) || 0)),
        sdStrokeWidth: Math.max(0, Math.min(40, Number(sdStrokeWidth) || 0)),
        sdStrokeColor: sdStrokeColor || '#FFD24AFF',
        ...
        // Accent bar under the LT box — applied display-side. Global values are set
        // per output by applyProjectionRuntimeSnapshot during the per-output build.
        ltBoxUnderlineEnabled: !!ltBoxUnderlineEnabled,
        ltBoxUnderlineColor: ltBoxUnderlineColor || '#FFD500',
        ltBoxUnderlineThickness: Math.max(0, Math.min(40, Number(ltBoxUnderlineThickness) || 0)),
        // Per-side padding (px) and the fit guarantee — both consumed display-side, where
        // the real box and the rendered text can actually be measured.
        padTopFull, padRightFull, padBottomFull, padLeftFull,
        padTopLT, padRightLT, padBottomLT, padLeftLT,
        forceFitToBox: !!forceFitToBox,
        minFitScale: Math.max(0.02, Math.min(1, Number(minFitScale) || 0.03)),
    ...
    // Renders the live payload once per active output, each under that output's own
    // design profile, and ships them in ONE message.
    //
    // One message, not one per output: N messages would land at N different instants
    // and screens would tear apart on fast transitions. Here the content is selected
    // once inside buildLivePayload() and every screen receives the same verse in the
    // same message — differing only by which entry of `designs` it reads.
    //
    // The flat fields stay the default/OBS design, so BSP_display.html and any existing
    // browser source keep working with no change.
    // Keeps only the keys where an output's payload differs from the flat base. The
    // display merges this diff over the flat fields (bspResolveDesignedPayload), so the
    // result is identical to sending the whole payload — but a handful of changed keys
    // travels instead of a full duplicate of the verse/song per screen. This is the main
    // per-message size win for multiple remote screens.
    function bspDiffPayload(payload, base) {
      const diff = {};
      Object.keys(payload).forEach((k) => {
        if (payload[k] === undefined) return;
        if (JSON.stringify(payload[k]) !== JSON.stringify(base[k])) diff[k] = payload[k];
      });
      return diff;
    }
    ...
    function pushLiveUpdate() {
      // ═══ Décochage Option B ═══
      // Quand le modal Settings édite un ÉCRAN, les globales d'exécution portent le design de
      // CET écran (pour que chaque contrôle existant l'édite via le chemin de capture). Or la
      // sortie Live/OBS — et chaque diff par-sortie — doit rester calculée sur le profil de
      // BASE. Uniquement dans ce cas, on bascule les globales vers la base du kind à l'antenne
      // pour toute la construction, puis on restaure dans le finally. En fonctionnement normal
      // (édition du Live, ou modal sur Live) les globales SONT déjà la base : rien n'est
      // basculé, le comportement est inchangé.
      const bspEditingScreen = (typeof bspSettingsOutputIsScreen === 'function') && bspSettingsOutputIsScreen();
      let bspSavedRuntime = null;
      if (bspEditingScreen && typeof captureProjectionSettingsSnapshot === 'function' &&
          typeof ensureProjectionSettingsProfiles === 'function' && typeof applyProjectionRuntimeSnapshot === 'function') {
        const liveKind = (typeof livePointer !== 'undefined' && livePointer && livePointer.kind === 'bible') ? 'bible' : 'songs';
        const baseProfile = ensureProjectionSettingsProfiles()[liveKind];
        if (baseProfile) {
          bspSavedRuntime = captureProjectionSettingsSnapshot();
          applyProjectionRuntimeSnapshot(baseProfile);
        }
      }
      try {
        return bspPushLiveUpdateBody();
      } finally {
        if (bspSavedRuntime) applyProjectionRuntimeSnapshot(bspSavedRuntime);
      }
    }

    function bspPushLiveUpdateBody() {
      const basePayload = buildLivePayload();
      if (!basePayload) return false;

      // Build each designed output's FULL payload once (used for bookkeeping/freeze), then
      // transmit only its diff from the base.
      const fullByOutput = {};
      bspGetDesignedOutputs().forEach((output) => {
        fullByOutput[output.id] = bspBuildLivePayloadForProfile(output.profile) || basePayload;
      });

      // Record the FULL payload each output is about to show, so freezing later can pin what
      // is genuinely on screen. 'obs' and every known screen are recorded (an output with no
      // override still shows the base payload and must be freezable).
      bspLastSentDesigns[BSP_DEFAULT_OUTPUT_ID] = fullByOutput[BSP_DEFAULT_OUTPUT_ID] || basePayload;
      if (typeof bspScreensAdded !== 'undefined' && Array.isArray(bspScreensAdded)) {
        bspScreensAdded.forEach((s) => {
          if (s && s.id) bspLastSentDesigns[s.id] = fullByOutput[s.id] || basePayload;
        });
      }

      // Transmit diffs, not full duplicates. The flat fields already carry the base design,
      // so an output with no override needs no entry at all. (The old code also sent a
      // redundant designs['obs'] === basePayload — dropped here.)
      const designs = {};
      Object.keys(fullByOutput).forEach((id) => {
        designs[id] = bspDiffPayload(fullByOutput[id], basePayload);
      });

      // Frozen outputs replay their snapshot — a FULL payload of the OLD content, not a diff
      // of the current base, so it is sent whole.
      Object.keys(bspFrozenOutputs).forEach((id) => {
        if (bspFrozenOutputs[id]) designs[id] = bspFrozenOutputs[id];
      });

      // Blanked outputs (remote screen play/Stop): a tiny marker is enough — the display
      // clears when it sees __blankOutput after merging over the flat base.
      if (typeof bspScreensBlanked !== 'undefined' && bspScreensBlanked) {
        Object.keys(bspScreensBlanked).forEach((id) => {
          if (bspScreensBlanked[id]) designs[id] = { __blankOutput: true };
        });
      }

      postUpdate({ ...basePayload, designs });
      return true;
    }
