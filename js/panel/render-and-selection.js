    ...
    function renderVersionBar() {
      ...
      if (!bar) return;
      // The strip of per-version buttons is now a single dropdown declared in the
      // markup, so nothing is rebuilt here \u2014 the widget only re-reads `bibles` and
      // `activeBibleVersion`. Visibility of #version-bar is still owned by
      // setSidebarTab() (Bible tab only), which keeps working untouched.
      if (typeof bspSyncBibleVersionDropdown === 'function') bspSyncBibleVersionDropdown();
      updateBibleLists();
    ...
    async function changeActiveBibleVersion(ver) {
      ...
      // Lazy load: bring the newly active version's verses into memory before any
      // synchronous bibles[activeBibleVersion] read below, then free the versions that
      // are no longer active/secondary.
      if (typeof ensureBibleLoaded === 'function') await ensureBibleLoaded(ver);
    ...
    function updateBibleLists() {
      ...
      if (!activeBibleVersion || !bibles[activeBibleVersion]) {
        // Deleting the last version empties the list: the widget must follow, or it
        // keeps offering a stale book name that no longer resolves to anything.
        if (typeof bspSyncBibleCategoryDropdown === 'function') bspSyncBibleCategoryDropdown();
        return;
      }
      ...
      const appendBookOption = (parent, book) => {
      ...
      const groups = (typeof bspGroupBooksByCategory === 'function')
        ? bspGroupBooksByCategory(uniqueBooks)
        : [];
      ...
      if (groups.length) {
        groups.forEach(group => {
          ...
          optgroup.dataset.categoryId = group.id;
          group.books.forEach(book => appendBookOption(optgroup, book));
          bookSelect.appendChild(optgroup);
        });
      } else {
        uniqueBooks.forEach(book => appendBookOption(bookSelect, book));
      }
      ...
      window._openSidebarContextMenu = function(tab, item, itemIndex, arrayIndex, x, y, contextMeta) {
        ...
        if (isSchedule) {
          ...
          items.push({ label: t('common_remove_from_setlist'), icon: '✕', danger: true, action: () => {
            const nm = (schedule[arrayIndex] && schedule[arrayIndex].title) || t('delete_this_item');
            const doRemove = () => {
            ...
            if (typeof showConfirm === 'function') {
              showConfirm(t('setlist_remove_confirm_title'), t('setlist_remove_confirm_msg').replace('{name}', nm), doRemove);
            } else doRemove();
            close();
          }});
        }
    ...
    // ── Tri alphabétique des chants (A-Z puis #) ──
    // Appliqué UNIQUEMENT au chargement (boot / import). Un chant nouvellement ajouté est
    // simplement AJOUTÉ EN FIN de liste (les `songs.push` ne trient pas) et n'est reclassé
    // qu'au prochain rechargement — cela évite le bug de réorganisation « au milieu » de la
    // liste pendant l'édition (cf. note historique « pas de tri live »).
    function bspSongGroupPrefix(title) {
      const s = String(title || '').replace(/^\s+/, '');
      return /^\p{L}/u.test(s) ? '0' : '1'; // lettres (A-Z) d'abord, groupe # (chiffres/symboles) ensuite
    }
    function bspSortSongsAlphabetically() {
      if (typeof songs === 'undefined' || !Array.isArray(songs) || songs.length < 2) return;
      songs.sort((a, b) => {
        const ga = bspSongGroupPrefix(a && a.title), gb = bspSongGroupPrefix(b && b.title);
        if (ga !== gb) return ga < gb ? -1 : 1;
        return String((a && a.title) || '').localeCompare(String((b && b.title) || ''), undefined, { numeric: true, sensitivity: 'base' });
      });
    }
    window.bspSortSongsAlphabetically = bspSortSongsAlphabetically;

    // ── Lazy-load de la liste des chants (lots de 50) ──
    // Seule la liste des CHANTS est paginée (elle peut atteindre des dizaines de milliers
    // d'entrées). On rend un lot, puis 50 de plus quand une sentinelle en bas entre dans la vue.
    const BSP_LIST_PAGE = 50;
    let bspListRenderLimit = BSP_LIST_PAGE;
    let bspListRenderKey = '';
    let bspListLazyObserver = null;

    // Ligne SÉPARATEUR de section dans la setlist : bande titrée, RÉPLIABLE, renommable en
    // place, supprimable, glissable — mais NON sélectionnable/projetable. Les icônes sont de
    // vraies cibles cliquables espacées (26×26) pour ne plus être trop serrées.
    function bspBuildSetlistSectionRow(div, s, i) {
      // Nombre d'éléments regroupés sous cette section (jusqu'à la prochaine section).
      let childCount = 0;
      for (let j = i + 1; j < schedule.length; j++) { if (schedule[j] && schedule[j]._metaKind === 'section') break; childCount++; }
      const collapsed = !!s.collapsed;
      div.className = 'setlist-section-row' + (collapsed ? ' is-collapsed' : '');
      div.draggable = true;
      div.style.cssText = 'display:flex;align-items:center;gap:4px;margin:8px 0 2px;padding:4px 4px 4px 6px;background:linear-gradient(90deg,rgba(107,122,148,.22),rgba(107,122,148,.04));border-left:3px solid var(--accent);border-radius:4px;color:var(--text)';

      // Fabrique une icône-bouton nette, espacée et facile à cliquer.
      const mkBtn = (txt, title, onClick, extraCss) => {
        const b = document.createElement('span');
        b.textContent = txt; b.title = title || ''; b.setAttribute('role', 'button');
        b.style.cssText = 'flex:none;display:inline-flex;align-items:center;justify-content:center;width:26px;height:26px;border-radius:5px;cursor:pointer;font-size:14px;line-height:1;opacity:.85;user-select:none;transition:background .12s;' + (extraCss || '');
        b.onmouseenter = () => { b.style.background = 'rgba(255,255,255,.14)'; b.style.opacity = '1'; };
        b.onmouseleave = () => { b.style.background = 'transparent'; b.style.opacity = '.85'; };
        b.onclick = (e) => { e.stopPropagation(); onClick(); };
        return b;
      };

      const label = document.createElement('span');
      label.className = 'setlist-section-row__label';
      label.style.cssText = 'flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:700;text-transform:uppercase;letter-spacing:.05em;font-size:11px';
      label.textContent = s.title || '';

      const startRename = () => {
        const input = document.createElement('input');
        input.type = 'text'; input.value = s.title || '';
        input.className = 'setlist-section-row__input';
        input.style.cssText = 'flex:1;min-width:0;font:inherit;font-weight:700;text-transform:uppercase;letter-spacing:.05em;font-size:11px;color:var(--text);background:rgba(0,0,0,.25);border:1px solid var(--accent);border-radius:3px;padding:2px 6px';
        let done = false;
        const commit = (save) => {
          if (done) return; done = true;
          if (save) { const v = input.value.trim(); if (v) s.title = v; }
      ...
      // Chevron : replier / déplier la section.
      const chev = mkBtn(collapsed ? '▸' : '▾',
        collapsed ? ((typeof t === 'function' && t('setlist_section_expand')) || 'Expand') : ((typeof t === 'function' && t('setlist_section_collapse')) || 'Collapse'),
        () => { s.collapsed = !s.collapsed; if (typeof saveState === 'function') saveState(); if (typeof saveToStorageDebounced === 'function') saveToStorageDebounced(); renderSongs(); });

      const count = document.createElement('span');
      count.style.cssText = 'flex:none;font-size:10px;font-weight:600;opacity:.55;min-width:16px;text-align:center';
      count.textContent = String(childCount);

      const rename = mkBtn('✎', (typeof t === 'function' && t('setlist_section_rename')) || 'Rename section', () => startRename());
      rename.className = 'setlist-section-row__rename';
      const del = mkBtn('✕', (typeof t === 'function' && t('common_delete')) || 'Delete',
        () => { schedule.splice(i, 1); if (typeof saveState === 'function') saveState(); if (typeof saveToStorageDebounced === 'function') saveToStorageDebounced(); renderSongs(); });
      const handle = mkBtn('☰', '', () => {}, 'cursor:grab');
      ...
      div.ondblclick = (e) => { e.stopPropagation(); startRename(); };
      // Drag & drop : mêmes règles que les lignes normales de la setlist.
      div.addEventListener('dragstart', () => { dragIndex = i; div.classList.add('dragging'); });
      div.addEventListener('dragend', () => { dragIndex = null; div.classList.remove('dragging'); });
    ...
    function renderSongs() {
      ...
      // Doublons de setlist : compte par clé (même chant/verset) pour l'insigne « dupliqué ».
      const scheduleDupCounts = new Map();
      if (sidebarTab === 'schedule' && buttonContextTab === 'schedule' && typeof getScheduleEntryKey === 'function') {
        schedule.forEach((it) => { const k = getScheduleEntryKey(it); if (k) scheduleDupCounts.set(k, (scheduleDupCounts.get(k) || 0) + 1); });
      }
      // Lazy-load (chants uniquement) : réinitialiser le lot quand l'onglet ou la recherche change.
      const lazyList = (sidebarTab === 'songs' && buttonContextTab === 'songs');
      const renderKey = sidebarTab + '|' + buttonContextTab + '|' + q;
      if (renderKey !== bspListRenderKey) { bspListRenderKey = renderKey; bspListRenderLimit = BSP_LIST_PAGE; }
      let bspShownCount = 0, bspHiddenAfterLimit = 0;
      const scheduleUi = (sidebarTab === 'schedule' && buttonContextTab === 'schedule');
      let bspCollapsedUnder = false; // vrai tant qu'on est sous une section repliée
      const frag = document.createDocumentFragment();
      const navMirrorItems = [];
      displayList.forEach((s, i) => {
        if (q && !getItemSearchableText(s).includes(q)) return;
        if (lazyList) { bspShownCount++; if (bspShownCount > bspListRenderLimit) { bspHiddenAfterLimit++; return; } }
        // Section repliée : masquer les éléments jusqu'à la prochaine section (les sections
        // elles-mêmes restent toujours visibles ; la barre gère l'état).
        if (scheduleUi && s && s._metaKind !== 'section' && bspCollapsedUnder) return;
        ...
        // Section separator (setlist only) — its own row, not selectable/projectable.
        if (s && s._metaKind === 'section' && scheduleUi) {
          bspCollapsedUnder = !!s.collapsed; // les éléments suivants sont masqués si repliée
          bspBuildSetlistSectionRow(div, s, i);
          frag.appendChild(div);
          navMirrorItems.push({ row: div, title: s.title || '', snippet: '', active: false, activate: () => {} });
          return;
        }
        ...
        if (isScheduleUi) {
          // Insigne « dupliqué » : cet élément figure plusieurs fois dans la setlist.
          const dupKey = (typeof getScheduleEntryKey === 'function') ? getScheduleEntryKey(s) : null;
          if (dupKey && scheduleDupCounts.get(dupKey) > 1) {
            const badge = document.createElement('span');
            const dupLabel = (typeof t === 'function' && t('setlist_dup_badge') && t('setlist_dup_badge') !== 'setlist_dup_badge') ? t('setlist_dup_badge') : 'Dupliqué';
            badge.textContent = dupLabel;
            badge.title = (typeof t === 'function' && t('setlist_dup_badge_title') && t('setlist_dup_badge_title') !== 'setlist_dup_badge_title') ? t('setlist_dup_badge_title') : dupLabel;
            badge.style.cssText = 'font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:#ffb020;background:rgba(255,176,32,.15);border:1px solid rgba(255,176,32,.45);border-radius:4px;padding:1px 6px;margin-right:10px;white-space:nowrap;flex:none';
          ...
          // Songs get a visible ✎ to rename the title in place (the right-click menu also
          // does it, but a button is discoverable). Bible verses aren't renameable.
          if (sidebarTab === 'songs') {
            const edit = document.createElement('span');
            edit.className = 'song-item__rename';
            edit.style.cursor = 'pointer'; edit.style.marginRight = '10px'; edit.textContent = '✎';
            edit.title = t('common_rename_song');
            edit.setAttribute('aria-label', t('common_rename_song'));
            edit.onclick = (e) => { e.stopPropagation(); bspStartSongTitleEdit(div, left, itemIndex); };
        ...
        if (sidebarTab === 'schedule' && buttonContextTab === 'schedule') {
          ...
          // Double-clicking a saved song's title edits it in place too.
          if (sidebarTab === 'songs') {
            left.style.cursor = 'text';
            left.ondblclick = (e) => { e.stopPropagation(); bspStartSongTitleEdit(div, left, itemIndex); };
          }
        }
      ...
      // Lazy-load : sentinelle en bas de liste → charge 50 chants de plus quand elle approche.
      if (bspListLazyObserver) { try { bspListLazyObserver.disconnect(); } catch (_) {} bspListLazyObserver = null; }
      if (lazyList && bspHiddenAfterLimit > 0) {
        const sentinel = document.createElement('div');
        sentinel.className = 'bsp-list-sentinel';
        sentinel.style.cssText = 'height:1px;width:100%';
        list.appendChild(sentinel);
        if (typeof IntersectionObserver === 'function') {
          // Ancrer l'observateur sur le conteneur réellement défilable (sinon root=viewport peut
          // ne pas déclencher quand la liste défile à l'intérieur d'une colonne à overflow).
          let scrollRoot = null, p = list.parentElement;
          while (p) { const oy = getComputedStyle(p).overflowY; if (oy === 'auto' || oy === 'scroll') { scrollRoot = p; break; } p = p.parentElement; }
          bspListLazyObserver = new IntersectionObserver((entries) => {
            if (entries.some(e => e.isIntersecting)) { bspListRenderLimit += BSP_LIST_PAGE; renderSongs(); }
          }, { root: scrollRoot, rootMargin: '400px 0px' });
          bspListLazyObserver.observe(sentinel);
        } else {
          bspListRenderLimit = Number.MAX_SAFE_INTEGER; // repli : pas d'IO → tout afficher
        }
      }
      ...
      for (const file of input.files) {
        try {
          ...
          if (name.endsWith('.xml')) {
            ...
            // A .json dropped on the toolbar Import button is a setlist or a full config —
            // route it to the same handlers the Plugin-Settings buttons use, so importing
            // a setlist works from the interface too, not only from Settings.
            let obj = null;
            try { obj = JSON.parse(text); } catch (_) {}
            const kind = obj && obj.kind;
            const looksSetlist = obj && obj.data && Array.isArray(obj.data.setlist);
            if (kind === 'config' && typeof bspHandleConfigImport === 'function') {
              await bspHandleConfigImport({ files: [file], value: '' });
            } else if ((kind === 'setlist' || looksSetlist) && typeof bspHandleSetlistImport === 'function') {
              await bspHandleSetlistImport({ files: [file], value: '' });
            } else {
              skippedUnsupported += 1;
            }
            continue; // those handlers show their own confirmation/toast
          } else {
            skippedUnsupported += 1;
            continue;
          }
    ...
    async function handleSearchEnter(e) {
      ...
      if (!activeBibleVersion || !(activeBibleVersion in bibles)) return;
      // Lazy load: the active version may still be a sentinel on first use.
      if (typeof ensureBibleLoaded === 'function') await ensureBibleLoaded(activeBibleVersion);
      if (!isBibleLoaded(activeBibleVersion)) return;
    ...
    function selectItem(i, opts = {}) {
      // Commit any pending debounced lyric edit on the outgoing item before we switch,
      // so its search index / persistence are up to date and nothing typed is lost.
      if (typeof window.bspFlushSongEdit === 'function') window.bspFlushSongEdit();
