    ...
    function getStandaloneOutputUrl() {
      const base = getDisplayFileForTab('lyrics', 'standalone');
      // Tell the window which output it is, so it can pick its entry out of the
      // `designs` map. Without this it falls back to the default/OBS design.
      const id = String(bspOutputScreenOverride || '').trim();
      if (!id) return base;
      return base + (base.includes('?') ? '&' : '?') + 'outputId=' + encodeURIComponent(id);
    }
...
<body>
  ...
  <\/script>
    ...
    function getOutputBoundsFromSelection() {
      ...
      // The override wins: see bspOutputScreenOverride in panel-app-core.js for why the
      // select cannot be trusted to carry the choice.
      const selected = String(bspOutputScreenOverride || select?.value || 'auto');
    ...
    async function refreshOutputScreenList(opts = {}) {
      ...
      outputScreenList = [];
      // Application desktop (Electron) : la liste des moniteurs vient du processus
      // principal (screen.getAllDisplays). Ids POSITIONNELS `screen_<n>` — identiques
      // au schéma des liens OBS et aux designs déjà enregistrés — plus le vrai id
      // Electron (desktopDisplayId) conservé pour placer la fenêtre plein écran.
      if (window.bspDesktop && window.bspDesktop.isElectron) {
        try {
          const displays = await window.bspDesktop.listDisplays();
          outputScreenList = (Array.isArray(displays) ? displays : []).map((d, idx) => ({
            id: `screen_${idx}`,
            label: d.label || (d.primary ? 'Primary Display' : `Display ${idx + 1}`),
            left: Number((d.bounds && d.bounds.x) || 0),
            top: Number((d.bounds && d.bounds.y) || 0),
            width: Number((d.bounds && d.bounds.width) || 1280),
            height: Number((d.bounds && d.bounds.height) || 720),
            isPrimary: !!d.primary,
            isInternal: !!d.internal,
            desktopDisplayId: String(d.id)
          }));
        } catch (_) { outputScreenList = []; }
        if (meta) {
        ...
        if (outputPopoverOpen) renderOutputScreenPopover();
        return;
      }
      if (hasDesktopOutputBridge()) {
        try {
