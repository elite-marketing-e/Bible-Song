    ...
    function syncAppStateFromUi() {
      ...
      appState = {
        ...
        setlistMeta: { title: (setlistMeta && setlistMeta.title) || '', comment: (setlistMeta && setlistMeta.comment) || '' },
        subtitles: (typeof bspSubtitlesExport === 'function') ? bspSubtitlesExport() : (Array.isArray(bspSubtitles) ? bspSubtitles : []),
    ...
    function renderFontFamilyOptions(selectedValue) {
      ...
      select.innerHTML = '';
      // Built-in + uploaded + system fonts, via the shared builder so the global picker
      // and the per-screen picker always offer exactly the same list.
      const options = (typeof bspAllFontOptions === 'function')
        ? bspAllFontOptions()
        : [...baseFontOptions, ...customFonts.map(f => ({ label: f.name, value: `'${f.name}',sans-serif` }))];
      options.forEach(opt => {
    ...
    function saveState(label) {
      // Every meaningful mutation passes through here — the ideal spot to keep the
      // control phone in step with the panel (debounced + no-op when unchanged).
      ...
      const state = {
        ...
        setlistMeta: { title: (setlistMeta && setlistMeta.title) || '', comment: (setlistMeta && setlistMeta.comment) || '' },
        subtitles: (typeof bspSubtitlesExport === 'function') ? bspSubtitlesExport() : (Array.isArray(bspSubtitles) ? JSON.parse(JSON.stringify(bspSubtitles)) : []),
    ...
    function setupSidebarResizePersistence() {
      ...
      function restoreSidebarWidth() {
        ...
        // Re-anchor after restoring a saved width, or the tab starts at the default offset.
        if (typeof positionSidebarQuickActions === 'function') positionSidebarQuickActions();
      }
      restoreSidebarWidth();
      if (hHandle) {
        ...
        hHandle.addEventListener('pointermove', (e) => {
          ...
          // The Quick tab is position:fixed and anchored to #main's left edge from JS, so
          // it has to be repositioned as the sidebar is dragged — otherwise it stays put
          // and detaches from the sidebar it belongs to.
          if (typeof positionSidebarQuickActions === 'function') positionSidebarQuickActions();
        });
    ...
    function switchSettingsTab(tabId) {
      ...
      if ((tabId === 'sidescreen' || tabId === 'typography') && typeof bspRenderSettingsDesignPages === 'function') {
        bspRenderSettingsDesignPages();
      }
    }
    ...
    function applyLoadedState(stateValue, songRecords, bibleRecords, opts = {}) {
      ...
      // Tri alphabétique appliqué AU CHARGEMENT uniquement (A-Z puis #) : un chant ajouté en
      // cours de session reste en fin de liste et n'est reclassé qu'au prochain rechargement.
      if (typeof bspSortSongsAlphabetically === 'function') bspSortSongsAlphabetically();
      // Lazy bibles: every version becomes a key so Object.keys(bibles) still lists them
      // all, but only records that arrived WITH parsed verses (the boot preload, a relay
      // push, or an import) hold data. The rest are null sentinels, loaded on demand by
      // ensureBibleLoaded(). See the lazy-loading block in state-and-storage.js.
      bibles = {};
      bibleRecords.forEach(record => {
        if (!record || !record.id) return;
        bibles[record.id] = Array.isArray(record.parsedData) ? record.parsedData : null;
      });
      ...
      const savedMeta = appState.setlistMeta && typeof appState.setlistMeta === 'object' ? appState.setlistMeta : {};
      setlistMeta = { title: String(savedMeta.title || ''), comment: String(savedMeta.comment || '') };
      if (typeof bspSubtitlesLoad === 'function') bspSubtitlesLoad(appState.subtitles);
      ...
      // Key-existence, not value-truthiness: a restored active version is a valid choice
      // even while it is still a sentinel (null). Using `!bibles[x]` here would wrongly
      // bump a lazy-unloaded active version to versions[0].
      if (!activeBibleVersion || !(activeBibleVersion in bibles)) {
        const versions = Object.keys(bibles);
        if (versions.length > 0) activeBibleVersion = versions[0];
      }
      // The chosen active version (and dual secondary) must actually be in memory before
      // the synchronous render below reads bibles[activeBibleVersion]. The boot preload
      // normally already did this; ensure it as a safety net (no-op if loaded), then
      // refresh the bible views once it resolves in case the preload missed it.
      if (activeBibleVersion && !isBibleLoaded(activeBibleVersion) && typeof ensureBibleLoaded === 'function') {
        ensureBibleLoaded(activeBibleVersion).then(() => {
          if (typeof renderVersionBar === 'function') renderVersionBar();
          if (sidebarTab === 'bible' && typeof updateBibleLists === 'function') {
            updateBibleLists();
            if (typeof ensureSelectionFallback === 'function') ensureSelectionFallback();
          }
        });
      }
    ...
    async function bootApp() {
      await openDb();
      const [stateEntry, songRecords, bibleKeys, ltStylesEntry, bgSettingsEntry, animationSettingsEntry, typographySettingsEntry, modeSettingsEntry] = await Promise.all([
        idbGet(STORE_STATE, 'appState'),
        idbGetAll(STORE_SONGS),
        idbGetAllKeys(STORE_BIBLES),   // names only — verses load lazily (out-of-memory fix)
      ...
      // Preload ONLY the version(s) the panel opens on: the restored active version and,
      // if dual mode was on, its secondary. Every other version boots as a sentinel and
      // is fetched the first time it is selected. This is what keeps boot memory to one
      // or two versions instead of the whole library.
      const st0 = (stateEntry && stateEntry.value) ? stateEntry.value : null;
      const keyList = Array.isArray(bibleKeys) ? bibleKeys.map(String) : [];
      let activeName = (st0 && (st0.selectedBibleId || (st0.bibleNav && st0.bibleNav.versionId))) || '';
      if (!activeName || keyList.indexOf(activeName) === -1) activeName = keyList[0] || '';
      const secondaryName = (st0 && st0.settings && st0.settings.dualVersionModeEnabled && st0.settings.dualVersionSecondaryId) || '';
      const preloadNames = [...new Set([activeName, secondaryName].filter(n => n && keyList.indexOf(n) !== -1))];
      const preloaded = {};
      await Promise.all(preloadNames.map(n => idbGet(STORE_BIBLES, n).then(r => { if (r) preloaded[n] = r; }).catch(() => {})));
      // Records for applyLoadedState: preloaded ones carry parsedData; the rest are id-only
      // stubs, which applyLoadedState turns into null sentinels.
      const bibleRecords = keyList.map(id => preloaded[id] || { id });
    ...
    async function initControlPanel() {
      ...
      restoreVmixSettingsUi();
      applyHostModeUi();
      // Bind before bootApp(): bootApp restores state and rebuilds the book/version
      // lists, which sync these widgets.
      bspInitBibleCategoryDropdown();
      bspInitBibleVersionDropdown();
      bspOutputDesignsRestore();
      if (typeof bspRestoreSystemFonts === 'function') bspRestoreSystemFonts();
      if (typeof bspLoadMediaBank === 'function') bspLoadMediaBank().catch(() => {});
      if (typeof bspInitMediaGallery === 'function') bspInitMediaGallery();
      bspInitScreensBar();
      bspInitScreenDesignEditor();
      if (typeof bspInitScreenModeZone === 'function') bspInitScreenModeZone();
      if (typeof bspInitLivePreview === 'function') bspInitLivePreview();
