    ...
    function getEffectiveLiveBackgroundState() {
      const panelKind = getCurrentPanelContentKind();
      // Skipped while rendering another output's design: this line writes the panel UI
      // into the shared live background state, and a per-output pass must leave no trace
      // behind. The profile branch below supplies that output's background.
      if (!bspDesignProfileOverride && isLive && livePointer && panelKind && livePointer.kind === panelKind) {
      ...
      if (isLive && livePointer && livePointer.kind && typeof getProjectionSettingsSnapshotForTab === 'function') {
        ...
        if (profile && typeof profile === 'object') {
          ...
          return {
            ...
            // bspResolveMediaRef turns a "bank:<id>" reference into its data URL; a plain
            // URL or upload passes through unchanged.
            bgImage: ((profile.bgType || document.getElementById('bg-type')?.value || 'color') === 'image')
              ? ((profile.bgImageSource || document.getElementById('bg-image-source')?.value || 'upload') === 'upload'
                  ...
                  : bspResolveMediaRef((profile.bgImageUrl || '').trim()))
              : null,
            bgVideo: ((profile.bgType || document.getElementById('bg-type')?.value || 'color') === 'video')
              ? ((profile.bgVideoSource || document.getElementById('bg-video-source')?.value || 'upload') === 'upload'
                  ...
                  : bspResolveMediaRef((profile.bgVideoUrl || '').trim()))
              : null,
    ...
    function getEffectiveLiveTextTransformState() {
      const panelKind = getCurrentPanelContentKind();
      // While rendering another output's design, liveTextTransformState has been set to
      // that output's transforms; recapturing from the panel UI here would overwrite it.
      if (!bspDesignProfileOverride && isLive && livePointer && panelKind && livePointer.kind === panelKind) {
        liveTextTransformState = captureTextTransformStateFromUi();
      }
    ...
    function refreshProjectLiveButtonState() {
      ...
      // Keep the Live-screen chip's projecting state in step with the live button.
      ...
      pages.forEach((p, idx) => {
        ...
        btn.onclick = () => {
          ...
          pendingVerseProjectFrame = requestAnimationFrame(() => {
            pendingVerseProjectFrame = 0;
            // Selecting a slide stays on the current setlist entry.
        ...
        // Pencil: quick plain-text edit of this slide. Songs only — a Bible verse is not
        // the operator's text to rewrite. getIsBibleItem() only checks `version`, which a
        // verse added to the setlist doesn't carry, so `_metaKind` is checked too.
        const isBibleEntry = isBible || (currentItem && currentItem._metaKind === 'bible_verse');
        if (!isBibleEntry) {
          const pen = document.createElement('button');
          pen.className = 'lyric-edit';
          pen.type = 'button';
          pen.title = t('quick_edit_title');
          pen.setAttribute('aria-label', t('quick_edit_title'));
          pen.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4z"/></svg>';
          pen.onclick = (e) => {
            e.stopPropagation();
            if (typeof bspStartQuickEdit === 'function') bspStartQuickEdit(row, idx);
          };
          row.appendChild(pen);
        }
        // Explicit Live action, restored for the setlist workspace: send this slide and
        // move the setlist on to the next entry, the way the old per-item Live did.
        if (sidebarTab === 'schedule') {
          const goLive = document.createElement('button');
          goLive.className = 'lyric-go-live';
          goLive.type = 'button';
          goLive.textContent = 'Live';
          goLive.title = t('setlist_live_and_next');
          goLive.onclick = (e) => {
            e.stopPropagation();
            lineCursor = idx;
            updateActiveButtonInPlace(idx, isBible, bibleVersionIndex);
    ...
    // opts.advanceSetlist === false keeps the setlist selection where it is. Picking a
    // specific slide inside an item must not jump to the next setlist entry — that
    // "advance after live" step belongs to the explicit Live action, which is how the
    // operator says "this one is done, prepare the next".
    function projectLive(trigger = false, opts = {}) {
      if (trigger) isLive = true;
      if (!isLive) return;
      applyProgramDisplaySource('lyrics');
      if (!currentItem) return;
      if (currentItem._metaKind === 'section') return; // les sections ne se projettent pas
      const isBible = getIsBibleItem(currentItem);
      ...
      if (!isBible && activeRatio === 'custom') {
        ...
        const bgToggleEl = document.getElementById('bg-toggle');
      ...
      liveBackgroundState = captureBackgroundStateFromUi();
      // ═══ Mode de projection ROBUSTE ═══
      // On ne fait PLUS confiance à la globale transitoire `activeRatio` : elle pouvait être
      // désynchronisée du mode réellement choisi (remise à 'full' par l'ouverture/fermeture de
      // Settings, l'état workspace « focused » d'un autre kind, un reload…), si bien que le
      // premier verset/chant projeté après coup faisait `liveRatio = activeRatio` → retour en
      // Fullscreen. On lit le mode PERSISTÉ du kind projeté (celui qu'affiche la zone Mode et
      // que l'utilisateur a réglé), et on réaligne la globale dessus (auto-réparation).
      let projRatio = activeRatio;
      try {
        const projProfiles = (typeof ensureProjectionSettingsProfiles === 'function')
          ? ensureProjectionSettingsProfiles()
          : (typeof projectionSettingsProfilesByTab !== 'undefined' ? projectionSettingsProfilesByTab : null);
        const projKind = isBible ? 'bible' : 'songs';
        const persistedMode = projProfiles && projProfiles[projKind] ? projProfiles[projKind].activeRatio : null;
        if (persistedMode && ['full', '16-9', 'sd-left', 'sd-right'].indexOf(persistedMode) !== -1) projRatio = persistedMode;
      } catch (_) {}
      activeRatio = projRatio;
      liveRatio = projRatio;
      if (typeof bspSyncFocusedWorkspaceRatioForTab === 'function') bspSyncFocusedWorkspaceRatioForTab(isBible ? 'bible' : 'songs', projRatio);
      liveTextTransformState = captureTextTransformStateFromUi();
      if (projRatio === '16-9') {
        if (isBible) ensureBibleLtTextTransformNone({ force: true });
        else ensureSongLtTextTransformDefault({ force: true });
      }
      if (isBible && projRatio === '16-9') {
      ...
      if (isScheduleItem) {
        ...
        if (opts.advanceSetlist !== false &&
            setlistBehavior.advancePreviewAfterLive && scheduleProjectIndex < (schedule.length - 1)) {
          // Avancer jusqu'au prochain élément PROJETABLE en sautant les sections (séparateurs).
          let nextIndex = scheduleProjectIndex + 1;
          while (nextIndex < schedule.length && schedule[nextIndex] && schedule[nextIndex]._metaKind === 'section') nextIndex += 1;
